#!/usr/bin/env python3
"""
SuperSmartMatch V3.0 - Script de Test Massif MIGRÉ - VERSION CORRIGÉE
...
Auteur: SuperSmartMatch V3.0 Enhanced Migration - FIXED
"""
...
class SuperSmartMatchTesterV3:
    def __init__(self):
        ...
        print(f"🚀 SuperSmartMatch V3.0 Mass Tester initialisé - FIXED")
    ...
    def generate_reports_v3(self, results: List[Dict]) -> None:
        ...
        if csv_data:
            ...
            # Génère le rapport HTML V3.0 (FIXED)
            self.generate_html_report_v3_fixed(results, csv_data, timestamp)
        else:
            print("❌ Aucune donnée à exporter")

    def generate_html_report_v3_fixed(self, results: List[Dict], csv_data: List[Dict], timestamp: str) -> None:
        """🔧 FIXED - Génère un rapport HTML V3.0 avec gestion None values"""
        ...
        for item in csv_data:
            cv_metier = item["CV_Metier_Specifique"] or "unknown"  # 🔧 FIX: Handle None
            job_metier = item["Job_Metier_Specifique"] or "unknown"  # 🔧 FIX: Handle None
            score = item["Score"]
        ...
        # Distribution par métier spécifique (V3.0) - FIXED
        metier_stats = {}
        for item in csv_data:
            metier = item["CV_Metier_Specifique"] or "unknown"  # 🔧 FIX: Handle None
...
<body>
    <div class="header">
        ...
        <p>🔧 <strong>VERSION CORRIGÉE : Gestion des valeurs nulles</strong></p>
    </div>
    ...
    <div class="section">
        ...
        for i, match in enumerate(top_matches, 1):
            ...
            cv_metier = match["CV_Metier_Specifique"] or "unknown"  # 🔧 FIX
            job_metier = match["Job_Metier_Specifique"] or "unknown"  # 🔧 FIX
            html_content += f"""
            <tr>
                <td>{i}</td>
                <td>{match["CV"]}</td>
    ...
    <div class="section">
        ...
        for metier, stats in sorted(metier_stats.items(), key=lambda x: x[1]["avg_score"], reverse=True):
            metier_display = (metier or "unknown").replace('_', ' ').title()  # 🔧 FIX: Handle None
    ...
    <footer style="margin-top: 50px; text-align: center; color: #7f8c8d;">
        <p>Généré par SuperSmartMatch V3.0 Enhanced - 🎯 Précision Métier Fine - VERSION CORRIGÉE</p>
        ...
        html_file = f"rapport_matching_V3_FIXED_{timestamp}.html"
        with open(html_file, 'w', encoding='utf-8') as f:
            f.write(html_content)
        
        print(f"📊 Rapport HTML V3.0 CORRIGÉ généré: {html_file}")

    def run_mass_test_v3(self) -> None:
        """🚀 Lance le test massif V3.0 avec précision métier fine"""
        self.stats_v3["start_time"] = time.time()
        
        print("\n🚀 DÉBUT DU TEST MASSIF SUPERSMARTMATCH V3.0 - VERSION CORRIGÉE")
        ...
        print(f"\n✅ TEST MASSIF V3.0 TERMINÉ - VERSION CORRIGÉE!")
...
def main():
    ...
    print("🚀 SuperSmartMatch V3.0 - Mass Testing Tool - FIXED")
    print("🎯 RÉSOLUTION : Gestionnaire paie vs Assistant facturation 90% → 25%")
    print("=" * 70)
